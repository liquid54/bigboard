...
export default function Header() {
    ...
    const tooltipLinks = [
        ...
        { label: 'Зовнішня реклама', href: '/outdoor-advertising' },
        { label: 'Таргетована реклама', href: '/targeted-advertising' },
        { label: 'Контекстна реклама', href: '/contextual-advertising' },
    ];
    ...
    return (
        <header
            className={`fixed top-0 left-0 w-full z-30 flex items-center justify-between px-4 py-4 max-w-8xl mx-auto transition-colors duration-300
              ${scrolled ? 'bg-[#8d3ac8]' : 'bg-[var(--color-bg)]'}`}
        >
            <div className={'max-w-[1321px] flex items-center justify-between mx-auto w-full'}>
            <Link
            href="/"
        ...
        >
            ...
            <nav className="hidden md:flex gap-8">
                {nav.map((item) => {
                    if (item.label === 'Додаткові послуги') {
                        return (
                            <div
                                key={item.label}
                            ...
                            >
                                <a
                                    href={item.href}
                                    className="font-gotham font-medium text-white text-lg transition px-4 rounded-[20px] pt-[10px] pb-[40px] group-hover:bg-white group-hover:text-black"
                                >
                                    {item.label}
                                </a>
                                <div
                                    className="absolute left-1/2 -translate-x-1/2 mt-[16px] hidden group-hover:flex px-4 py-2 bg-white text-black text-sm rounded-t-none rounded-[20px] shadow-lg z-50 whitespace-nowrap"
                                >
                                    <div className="flex items-center">
                                        {tooltipLinks.map((link, index) => (
                                            <div key={link.label} className="flex items-center">
                                                {index !== 0 && (
                                                    <span className="h-[40px] w-px bg-black/20 mx-2"/>
                                                )}
                                                <Link
                                                    href={link.href}
                                                    className="text-black py-[10px] px-[10px] text-[20px] font-[400] "
                    ...
                    if (item.label === 'Залишити заявку') {
                        return (
                            <a
                                key={item.label}
                                href={item.href}
                                className="font-gotham font-medium text-white text-lg transition"
            ...
            <div className="flex gap-4 items-center text-white text-2xl">
                <div
                    className="cursor-pointer hidden md:inline relative"
                ...
                >
                    ...
                    {showPhoneTooltip && (
                        <a
                            ...
                            className="absolute top-[25px] mb-2 left-1/2 -translate-x-1/2
                             bg-white text-black rounded-full px-4 py-2 text-lg font-medium shadow-md flex items-center gap-2 whitespace-nowrap z-50"
